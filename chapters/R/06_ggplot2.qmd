# Rendere visibili i dati con `ggplot2` {#sec-r-ggplot2}
...
È utile avere letto @sec-r, @sec-r-programming e @sec-dplyr. Dovresti saper riconoscere l'unità di analisi, controllare la struttura di una tabella e leggere una pipeline come una sequenza di trasformazioni.
:::
...
## Dal quesito alla rappresentazione
...
## La struttura minima di un grafico
...
::: {.callout-important title="Il codice deve poter essere tradotto in una frase"}
...
## Confrontare gruppi senza nascondere i dati

Supponiamo di voler confrontare le ore di sonno tra gli anni di corso.

```{r}
#| label: fig-sleep-by-year
#| fig-cap: "Distribuzione delle ore di sonno per anno di corso."
...
::: {.callout-warning title="Un punto distante non è automaticamente un errore"}
...
```{r}
#| label: fig-stress-sleep-trend
#| fig-cap: "Relazione tra stress e qualità del sonno con una sintesi lineare."
...
::: {.callout-important title="Vedere un'associazione non significa vedere una causa"}
...
## Colori e pannelli devono rappresentare una domanda
...
## Etichette: descrivere ciò che viene mostrato

Un grafico dovrebbe poter essere compreso senza conoscere i nomi interni delle colonne.

```{r}
#| label: fig-labelled-example
#| fig-cap: "Grafico con etichette che descrivono variabili e unità."
...
## Quante osservazioni entrano nel grafico?
...
```{r}
#| label: ggplot2-complete-cases-check
...
## Controllare un grafico prodotto dall'AI
...
::: {.callout-tip title="Un prompt per ottenere un grafico semplice"}
> Ho un data frame R chiamato `studenti`, con una riga per studente. `stress` e `qualita_sonno` sono punteggi numerici; `anno` è una variabile categoriale ordinata.  
>
...
## Salvare il grafico in modo riproducibile

Un grafico può essere assegnato a un oggetto:

```{r}
#| label: ggplot2-graph-object
