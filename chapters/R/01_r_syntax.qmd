...
```
Quando incontri errori come questo, chiedi a AI la spiegazione del messaggio. Un errore comune è anche:
...
In questo corso useremo lo stile più diffuso, *Snake Case*, che separa le parole con il trattino basso `_`.
...
I commenti servono a spiegare *perché* il codice è scritto in un certo modo, non *come* funziona (questo è evidente leggendo il codice). Una buona pratica consiste nel commentare le decisioni o i passaggi che non risultano immediatamente evidenti. 
...
- facilitare il *debugging* e migliorare il riutilizzo del codice.  
...
```
Questo codice crea un oggetto chiamato `my_vec` che contiene una sequenza di numeri. Alcuni concetti fondamentali sulle funzioni in R sono le seguenti:
...
## Operazioni vettoriali e vettorializzazione in R
...
Il risultato è un nuovo vettore in cui ciascun elemento è stato moltiplicato per 5. La logica della vettorializzazione può essere riassunta nella tabella seguente.

```{r}
#| echo: false
#| tbl-cap: "Tabella creata dinamicamente con R e gt."
...
tabella_dati <- data.frame(
  Indice = i,
  Elemento = my_vec,
  Operazione = paste(my_vec, "× 5"),
  Risultato = risultato
)

# Creazione della tabella con gt
gt(tabella_dati) |>
  cols_align(align = "center") |> 
  tab_header(
    title = "Moltiplicazione degli Elementi di un Vettore"
  ) |>
  cols_label(
    Indice = "Indice i",
    Elemento = "Elemento my_vec[i]"
  )
```

L’operazione vettorializzata viene eseguita automaticamente su ciascun elemento del vettore, senza bisogno di cicli `for`.
...
```{r}
#| echo: false
#| tbl-cap: "Somma degli elementi di due vettori."

# 1. Dati di partenza
my_vec <- c(3, 5, 7, 1, 9, 20)
my_vec2 <- c(17, 15, 13, 19, 11, 0)

# 2. Creazione del tibble (simile a un data.frame)
dati_tabella <- tibble(
  indice = 1:length(my_vec),
  vec1 = my_vec,
  vec2 = my_vec2,
  operazione = paste(my_vec, "+", my_vec2),
  risultato = my_vec + my_vec2
)

# 3. Creazione e formattazione della tabella con gt
gt(dati_tabella) |>
  tab_header(
    title = "Operazioni di Somma tra Vettori",
    subtitle = "Ogni elemento di my_vec viene sommato al corrispondente elemento di my_vec2."
  ) |>
  tab_spanner(
    label = "Vettori di Input",
    columns = c(vec1, vec2)
  ) |>
  cols_label(
    indice = "Indice i",
...
2. Grazie all'aritmetica vettorializzata, quando sommiamo i vettori $\text{item1}$, $\text{item2}$, $\text{item3}$, $\text{item4}$, $\text{item5}$, R somma elemento per elemento:
   ...
   $$
   \text{total_scores}_i = \text{item1}_i + \text{item2}_i + \text{item3}_i + \text{item4}_i + \text{item5}_i
   $$
...
Per concludere, saper maneggiare i vettori è una competenza indispensabile in R. Operazioni come selezionare elementi, modificarli, ordinarli o gestire i valori mancanti non sono semplici esercizi: sono le fondamenta di qualsiasi analisi dei dati efficace in questo linguaggio.
...
R è molto più di un semplice linguaggio per la statistica: è una filosofia di lavoro che si fonda su tre principi fondamentali: **apertura**, **collaborazione** e **avanzamento della conoscenza**.

Cogliere questa filosofia è essenziale per chiunque utilizzi R, in qualsiasi ambito. R non sviluppa solo competenze tecniche, ma promuove una cultura di **ricerca trasparente e riproducibile**, pilastro di una scienza credibile.

**Open Source**  
R è open source: il suo codice è liberamente accessibile, modificabile e distribuibile. Questo garantisce trasparenza, favorisce la collaborazione e lo rende accessibile a tutti, ovunque, senza costi di licenza. La sua natura aperta lo arricchisce grazie ai contributi di una comunità globale attiva e diversificata.

**Comunità che collabora**  
Uno dei punti di forza di R è proprio la sua comunità. Statistici, ricercatori e *data scientist* di tutto il mondo contribuiscono sviluppando e condividendo pacchetti, ovvero insiemi di funzioni, dati e codice che estendono le possibilità del linguaggio. Oggi migliaia di pacchetti coprono quasi ogni area applicativa, rendendo R uno strumento flessibile e in continua evoluzione.

**Per una ricerca riproducibile**  
La ricerca riproducibile permette di ripetere uno studio utilizzando gli stessi dati e metodi, consentendo la verifica dei risultati e la costruzione di nuova conoscenza su basi solide.  
R supporta questo approccio con:  

- un ecosistema integrato di pacchetti per analisi e reporting; 
- strumenti come **R Markdown** e **Quarto**, che combinano in un unico documento testo descrittivo, codice, analisi e output. Questo permette di documentare ogni passo del processo di ricerca, garantendo trasparenza e facilitando la replicabilità.

In sintesi, comprendere la filosofia open source e il ruolo di R nella ricerca riproducibile chiarisce il motivo per cui è diventato uno strumento centrale nella scienza dei dati. Per i ricercatori in psicologia, utilizzare R significa produrre risultati più trasparenti, solidi e credibili, contribuendo così a una conoscenza scientifica più robusta e affidabile.
