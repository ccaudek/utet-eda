...
Questo capitolo potrebbe sembrare fuori luogo in un sito dedicato all'analisi esplorativa dei dati. Perché parlare di modelli cognitivi quando stiamo ancora imparando a calcolare le medie e a visualizzare le distribuzioni?

La risposta sta in una distinzione fondamentale che attraversa l'intera ricerca psicologica: la differenza tra "descrizione" e "spiegazione" di un fenomeno. L'EDA, che abbiamo presentato nei capitoli precedenti, è uno strumento potente per la descrizione: ci dice cosa accade nei dati. Ma la psicologia, in quanto scienza, aspira a qualcosa di più: *capire perché accade*.

Questo capitolo introduce l'idea che, per passare dalla descrizione alla spiegazione, sono necessari dei *modelli*, ovvero delle rappresentazioni formali dei meccanismi che generano il comportamento osservato. Non è necessario padroneggiare questi strumenti ora, ma è importante comprendere la direzione verso cui ci stiamo muovendo.  L'inferenza bayesiana, trattata nel manuale *Inferenza bayesiana in psicologia: Ragionare con l'incertezza*, fornisce il framework per stimare i parametri di questi modelli e valutare quale spiegazione sia più plausibile alla luce dei dati.

In altre parole: l'EDA ci mostra i pattern, i modelli spiegano cosa li produce, e l'inferenza bayesiana ci permette di scegliere tra spiegazioni alternative. Questo capitolo anticipa il terzo elemento, preparando il terreno per il passaggio al manuale principale.
:::
...
## Differenza epistemica: distinguere tra spiegazione e predizione

Un aspetto cruciale per distinguere tra spiegazioni fenomenologiche e meccanicistiche risiede nel fatto che solo queste ultime soddisfano i criteri propri del *potere esplicativo genuino*. Come evidenzia @povich_mechanistic_2025, un modello esplicativo deve permettere di:
...
## Cosa rende un modello *meccanicistico*?
...
Per approfondire questa distinzione, è utile richiamare i tre livelli di analisi proposti da @marr2010vision, un punto di riferimento classico nella psicologia cognitiva computazionale:
