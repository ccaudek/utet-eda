...
Per i dati riportati nel diagramma della @fig-cesd-bdi2, la covarianza è 207.4. Il segno positivo della covarianza ci dice che tra le due variabili c'è un'associazione lineare positiva. Per capire quale sia l'intensità della relazione lineare calcoliamo la correlazione. Essendo le deviazioni standard del BDI-II e del CES-D rispettavamente uguali a 15.37 e 14.93, la correlazione diventa uguale a $\frac{207.426}{15.38 \cdot 14.93} = 0.904.$ Tale valore è prossimo a 1.0, il che vuol dire che i punti del diagramma a dispersione non si discostano troppo da una retta con una pendenza positiva.
...
Il paradosso di Simpson si verifica quando, osservando i dati raggruppati per sottogruppi, si nota una certa relazione tra due variabili, ma aggregando i dati di tutti i sottogruppi insieme, emerge una relazione opposta. In altre parole, una tendenza presente all'interno di ciascun gruppo può scomparire o trasformarsi nel suo opposto quando i dati vengono combinati senza tenere conto della struttura dei sottogruppi.

Supponiamo di analizzare la relazione tra il voto di laurea ($X$) e la performance in un successivo programma di specializzazione ($Y$) in due dipartimenti universitari, A e B. All'interno di ciascun dipartimento, osserviamo una relazione positiva: gli studenti con voti di laurea più alti tendono, in media, a ottenere performance migliori nella specializzazione. Tuttavia, i due dipartimenti differiscono sistematicamente nei livelli medi:

* il **Dipartimento A** ha voti di laurea mediamente più bassi, ma performance nella specializzazione generalmente più elevate;
* il **Dipartimento B** ha voti di laurea mediamente più alti, ma performance nella specializzazione leggermente inferiori.

Quando i dati vengono aggregati ignorando l’appartenenza dipartimentale, queste differenze strutturali dominano la relazione globale, producendo una correlazione complessiva negativa tra $X$ e $Y$.

Questo fenomeno non è un'anomalia statistica, ma il risultato dell'interazione tra variabili confondenti e differenze sistematiche tra i gruppi.
...
* **All’interno del Dipartimento A**, la correlazione tra il voto di laurea ($X$) e la performance ($Y$) è positiva: a voti più alti corrispondono, in media, performance migliori.
* **All’interno del Dipartimento B**, la relazione è anch’essa positiva, indicando lo stesso andamento.
* **Considerando i due dipartimenti insieme**, la correlazione complessiva diventa negativa: ignorando l’appartenenza al dipartimento, sembra che voti di laurea più alti siano associati a performance peggiori.

Questa inversione apparente non riflette un cambiamento reale nel comportamento individuale, ma emerge perché il **dipartimento di appartenenza** agisce come variabile confondente, influenzando sia $X$ che $Y$.

#### Significato metodologico

Il paradosso di Simpson mostra in modo particolarmente chiaro che:

* le relazioni statistiche **non sono proprietà intrinseche dei dati**, ma dipendono dal livello di analisi;
* l’aggregazione indiscriminata può produrre conclusioni profondamente fuorvianti;
* è essenziale considerare la **struttura dei dati**, i **fattori di raggruppamento** e le **variabili confondenti** prima di trarre inferenze causali o interpretative.

In psicologia questo paradosso rappresenta un monito fondamentale: **analizzare senza stratificare equivale spesso a perdere l’informazione più rilevante**.

### Paradosso di Berkson

Il paradosso di Berkson è un fenomeno statistico che mostra come la selezione non casuale di un campione possa alterare l'associazione osservata tra due variabili, invertendone la direzione o generandone una spuria. Si verifica quando il dataset analizzato non è rappresentativo della popolazione generale, perché è sottoposto a un processo di selezione che dipende da entrambe le variabili in esame.

Un esempio classico è il seguente: analizzando solo ciclisti professionisti, potremmo non osservare alcuna relazione tra la capacità aerobica (VO2 max) e la probabilità di vincere una gara. Ciò accade perché tutti i ciclisti a quel livello hanno già superato una soglia elevatissima di VO2 max, rendendo irrilevante tale variabile all'interno del gruppo selezionato. Se invece considerassimo la popolazione generale, emergerebbe chiaramente una forte relazione positiva tra i due fattori.

Questo paradosso può essere generalizzato anche a situazioni psicologiche. Ad esempio, studiando l'associazione tra depressione e ansia esclusivamente in un campione di pazienti ricoverati in una struttura psichiatrica, si potrebbe osservare una correlazione nulla o negativa, poiché entrambe le condizioni sono già ampiamente rappresentate e il campione è preselezionato in base alla gravità del disagio. Nella popolazione generale, invece, le due condizioni mostrerebbero una chiara associazione positiva.

Il paradosso di Berkson evidenzia quindi l'importanza critica di interrogarsi sul processo di selezione dei dati e sull'adeguatezza del campione rispetto alla domanda di ricerca. Ignorare questo aspetto può condurre a conclusioni fuorvianti.

### Limiti delle statistiche riassuntive semplici

Un esempio famoso che dimostra i limiti delle semplici statistiche descrittive,  come la media, la deviazione standard e la correlazione, è il quartetto di Anscombe. Questo insieme di quattro piccoli dataset ha medie, varianze e correlazioni tra variabili identiche, ma rappresenta relazioni estremamente diverse tra $X$ e $Y$ [@anscombe1973graphs].

Importiamo il dataset `anscombe` disponibile nel pacchetto `datasets` di R:
...
Il dataset `anscombe` contiene quattro serie di dati (x1, y1), (x2, y2), (x3, y3) e (x4, y4), ognuna costituita da 11 coppie di valori (x, y). Per comprenderne meglio le loro caratteristiche, iniziamo calcolando alcune statistiche descrittive. La funzione `apply()` consente di applicare una funzione (ad esempio, `mean`) a tutte le colonne di un data frame. Utilizzandola sulle colonne di `anscombe`, otteniamo le medie di ciascuna delle otto variabili (le quattro x e le quattro y):
...
Se ci limitassimo a osservare queste statistiche (media, deviazione standard, correlazione), potremmo facilmente concludere che i quattro dataset sono sostanzialmente indistinguibili tra loro. Tuttavia, la realtà è ben diversa. Le statistiche descrittive, prese da sole, non offrono una visione completa e possono nascondere importanti differenze nella struttura dei dati.

Questa differenza diventa evidente non appena si decide di visualizzare i dati. La rappresentazione grafica fornisce informazioni che non emergono dalle sole statistiche riassuntive:
...
Osservando i grafici, notiamo subito che i quattro dataset sono molto diversi tra loro. 
...
- *Dataset 2:* sebbene la correlazione sia simile a quella del Dataset 1, ma i dati mostrano una relazione curvilinea e non lineare. La semplice correlazione lineare non riesce a coglierne la forma.
- *Dataset 3:* la presenza di un singolo outlier distorce la percezione della relazione lineare tra le variabili. La correlazione elevata è influenzata in modo sproporzionato da questo punto anomalo.
- *Dataset 4:* in questo caso, i dati non mostrano alcuna relazione lineare. La correlazione elevata è il risultato di un pattern fortemente atipico (per esempio, una sola coppia di punti allineati).

Il quartetto di Anscombe mette in luce un principio fondamentale: le statistiche descrittive, come la media, la deviazione standard e la correlazione, non sono sempre sufficienti per comprendere la natura dei dati. La visualizzazione grafica è essenziale per cogliere relazioni, pattern non lineari, outlier e altre caratteristiche che le statistiche descrittive non riescono a rivelare. In definitiva, questo esempio dimostra che l'analisi dei dati basata esclusivamente su poche statistiche di sintesi può condurre a conclusioni fuorvianti, mentre l'integrazione con la rappresentazione grafica offre una visione più completa e accurata.

## Riflessioni conclusive {.unnumbered .unlisted}

La covarianza e la correlazione sono strumenti fondamentali per descrivere la direzione e l’intensità di una relazione lineare tra due variabili. Tuttavia, la loro apparente semplicità non deve essere confusa con una completezza informativa. Una correlazione pari a zero non indica l’assenza di relazione in senso assoluto, ma segnala soltanto l’assenza di un legame **lineare**.

Inoltre, fenomeni come il **paradosso di Simpson** e il **paradosso di Berkson** mostrano chiaramente come le correlazioni possano risultare fuorvianti quando si ignorano la **struttura dei dati** o il **processo di selezione del campione**. In questi casi, la relazione osservata riflette più il modo in cui i dati sono stati raccolti o aggregati che il legame reale tra le variabili.

Esempi classici come il **quartetto di Anscombe** rafforzano ulteriormente questo punto, evidenziando l’importanza di affiancare alle statistiche riassuntive un’attenta **analisi grafica**. Solo così è possibile individuare pattern non lineari, valori anomali e peculiarità del dataset che le misure sintetiche da sole non riescono a catturare.

Nel prossimo capitolo, introdurremo approcci che permettono di andare oltre la semplice descrizione dell’associazione, avvicinandoci a una comprensione più profonda dei meccanismi sottostanti e ponendo le basi per un’analisi di tipo causale dei fenomeni psicologici.
