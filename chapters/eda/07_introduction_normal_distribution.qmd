...
#| fig-cap: "Effetto del parametro μ sulla forma della distribuzione normale"
p1
```

```{r}
#| echo: false
#| label: fig-normal-params-2
#| fig-cap: "Effetto del parametro σ sulla forma della distribuzione normale"
p2
```
...
p <- ggplot(df, aes(x = x, y = y)) +
  ...
  labs(
    x = "Deviazioni standard dalla media",
    y = "Densità",
    title = "La regola empirica 68-95-99.7",
    subtitle = "Percentuale di dati entro 1, 2 e 3 deviazioni standard"
  ) +
  # Aree (disegnale dal centro verso l'esterno per evitare effetti strani)
  geom_area(data = subset(df, x >= -3 & x <= 3), fill = "lightblue", alpha = 0.25) +
  geom_area(data = subset(df, x >= -2 & x <= 2), fill = "lightblue", alpha = 0.25) +
  geom_area(data = subset(df, x >= -1 & x <= 1), fill = "lightblue", alpha = 0.35) +
  ...
  # Forza limiti (soprattutto y) e lascia "respiro" sotto
  coord_cartesian(xlim = c(-3.5, 3.5), ylim = c(0, 0.42), clip = "off") +
  theme_minimal() +
  theme(
    plot.margin = margin(12, 12, 18, 12) # più margine in basso
  )
